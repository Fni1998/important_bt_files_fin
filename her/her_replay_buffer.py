...
class HerReplayBuffer(DictReplayBuffer):
    ...
    def __init__(
        ...
        self.metrics = np.zeros(self.buffer_size, dtype=np.float64)
    ...
    def sample_goals(
        ...
        elif self.goal_selection_strategy == GoalSelectionStrategy.PRIO:
            #replay with prioritisied transitions and episodes
            episode_indices_prio = np.zeros(len(her_episode_indices)).astype(int)
            inv_episode_indices = 1 - self.episodes_prio_sum
            inv_episode_indices = np.exp(inv_episode_indices - np.max(inv_episode_indices))
            inv_episode_indices = inv_episode_indices / np.sum(inv_episode_indices)
            episode_indices_prio = np.random.choice(np.arange(len(inv_episode_indices)), len(her_episode_indices), p=inv_episode_indices)
            self.goal_entropy = - sum(p * math.log2(p) for p in inv_episode_indices)  # trying to implement and check entropy
            transitions_indices_prio = np.zeros(len(her_episode_indices)).astype(int)
            inv_transition_prio = 1 - self.transitions_prio
            for i in range(0, len(inv_transition_prio)):
                inv_transition_prio[i] = np.exp(inv_transition_prio[i] - np.max(inv_transition_prio[i]))
                inv_transition_prio[i] = inv_transition_prio[i] / np.sum(inv_transition_prio[i])



            for j in range(0, len(her_episode_indices)):
                transitions_indices_prio[j] = np.random.choice(self.max_episode_length,p=inv_transition_prio[her_episode_indices[j]])

        else:
            raise ValueError(f"Strategy {self.goal_selection_strategy} for sampling goals not supported!")

        if self.goal_selection_strategy == GoalSelectionStrategy.PRIO:
            return self._buffer["achieved_goal"][episode_indices_prio, transitions_indices_prio]
        else:
    ...
    def _sample_transitions(
        ...
        if online_sampling:
            ...
            if self.full:
                ...
                if self.goal_selection_strategy == GoalSelectionStrategy.PRIO:
                    # read out of prio list
                    self.small_prio = np.zeros((self.pos, self.max_episode_length, 1))
                    self.small_prio[:self.pos, :self.max_episode_length, 0] = self._buffer['prio'][:self.pos,:self.max_episode_length, 0]
                    #small_prio has one dimension too much thats why we erase it here (50,20,1) to 50,20
                    self.small_prio = self.small_prio.squeeze()
                    # Priorities per Episode
                    self.episodes_prio_sum = np.sum(self.small_prio, axis=1)
                    #transform them into probabilities (implementation of softmax function)
                    self.episodes_prio_sum = np.exp(self.episodes_prio_sum / self.temperature - np.max(self.episodes_prio_sum / self.temperature))
                    self.episodes_prio_sum = self.episodes_prio_sum / np.sum(self.episodes_prio_sum)
                    #create new episode indices
                    episode_indices = np.random.choice(np.arange(len(self.small_prio)), batch_size, p=self.episodes_prio_sum)
                    #save the episode indices to make them available
                    self.ep_indices = episode_indices
                    #entropy of the episodes indices
                    self.ep_entropy = - sum(p * math.log2(p) for p in self.episodes_prio_sum)
        ...
        if online_sampling:
            #Transitionsampling
            if self.goal_selection_strategy == GoalSelectionStrategy.PRIO:
                self.transitions_prio = np.zeros((len(self.small_prio),self.max_episode_length))
                for i in range(0, len(self.small_prio) ):
                    self.transitions_prio[i] = np.exp(self.small_prio[i] - np.max(self.small_prio[i]))
                    self.transitions_prio[i] = self.transitions_prio[i] / np.sum(self.transitions_prio[i])

                transitions_indices = np.zeros(batch_size).astype(int)

                for j in range(0,batch_size):
                     transitions_indices[j] = np.random.choice(self.max_episode_length, p=self.transitions_prio[episode_indices[j]])
    ...
    def add(
        ...
        self._buffer["prio"][self.pos][self.current_idx] = infos[0]["Avg Difference"]


        # When doing offline sampling
        # Add real transition to normal replay buffer
        if self.replay_buffer is not None: #wir sampeln atm online also macht er das nicht!
